...
*   **Situation:** Developers were manually writing ad-hoc log statements within controller methods, creating a 30% disparity in audit records (missing user IDs, pre-states, timestamps) and causing compliance trace failures. Additionally, synchronous DB logging added latency and risked cascading transaction rollbacks.
...
*   **Action:** Designed a custom `@AuditLog` annotation powered by Spring AOP to intercept requests, dynamically extract execution context and state diffs via reflection, and asynchronously publish audit payloads to Kafka. A dedicated consumer then batches these into MongoDB.
